...
(0, reactLifecyclesCompat_es.polyfill)(Matrix);
...
var _Matrix2 = _interopRequireDefault(Matrix_1);
...
Object.defineProperty(exports, "default", {
  enumerable: true,
  get: function get() {
    return _uxcoreMatrix.default;
  }
});

var _uxcoreMatrix = interopRequireDefault(build);

module.exports = exports.default;
});

var Matrix$1 = unwrapExports(Matrix);
...
class treeMaxUtil {
  ...
  _flatDataSource() {
    const source = lodash.cloneDeep(this.source); // 1.转换数据源


    const transferSource = this._transferSource(source); // 2. 计算Y轴
  ...
  getResult() {
    // 1.获取数据转换后的扁平化
    const flatDataSource = this._flatDataSource(); // 2.计算不同类型的最大最小值范围
      ...
      if (_item.parent) {
        if (_item.isEnd || _item.type === this.types[this.types.length - 1]) {
          ...
          for (let i = prevIndex; i >= 0; i--) {
            ...
            const currentTypeIndex = lodash.indexOf(this.types, _item.type);

            const parentTypeIndex = lodash.indexOf(this.types, _parent.type); // 前一项与后一项类型相同，且起始点不一样
...
const TreeMatrix = ({
  ...
  const {
    ...
    const _treeMatrixData = new treeMaxUtil(data, types);
  ...
  const matrixCellWidth = react.useMemo(() => {
    let _cellWidth = [];
    types.forEach((type, index) => {
  ...
  return react__default.createElement(Matrix$1, _extends({}, newProps, {
...
exports.default = TreeMatrix;
//# sourceMappingURL=tree-matrix.js.map
